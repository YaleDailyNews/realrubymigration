...
class DataProcessor
  ...
  def mark_gallery_photos
    #adds the gallery photo IDs into the used in field for the photo
    #this prevents them from being dropped when we mark duplicates
    Galleryphoto.each do |gp|
      photo = Photo.where(el_id: gp.el_photo_id)
      next if photo.empty?
      photo = photo.first
      photo.used_in << gp.el_id
      photo.save!
    end
  ...
  def process_lead_photos(s)
    ...
    #
    #s -- a story object
    unless s.el_lead_photo_original_id.nil?
      lead_photo = Photo.where(el_id: s.el_lead_photo_original_id).first
  ...
  def transform_inlines(s)
    ...
    inlines.each do |inline|
      renderer = InlineRenderer.new(inline, parsed, s)
...
class InlineRenderer
  ...
  def initialize(cur_node, doc, story)
    @type = cur_node["type"] || ""
  ...
  def render_embedded
    ...
    if embed.nil?
      ...
      return nil
    elsif embed.el_embedded_object.include?("media.yaledailynews.com")
      return nil #don't render down inlines that refer to the dead media server. oops
    end

    embed.el_embedded_object.gsub!("http://yaledailynews.media.clients.ellingtoncms.com/", "http://yaledailynews.com/wp-content/uploads/legacy/media/") #remap soundslides URLs
  ...
  def render_photogallery
    ...
    gallery.wp_sites ||= []
    gallery.wp_sites << @story.wp_site
